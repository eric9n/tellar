...
fn collect_thread_files(dir: &Path, paths: &mut Vec<PathBuf>) -> anyhow::Result<()> {
    ...
    if dir_name == "history" || dir_name == "brain" {
        return Ok(());
    }
...
mod tests {
    ...
    fn test_should_ignore_rhythm_file_matches_knowledge_and_stream_logs() {
        assert!(should_ignore_rhythm_file(Path::new(
            "/tmp/rituals/KNOWLEDGE.md"
        )));
        assert!(should_ignore_rhythm_file(Path::new(
            "/tmp/rituals/2026-02-27.md"
        )));
        assert!(!should_ignore_rhythm_file(Path::new(
            "/tmp/rituals/deploy.md"
        )));
    }
